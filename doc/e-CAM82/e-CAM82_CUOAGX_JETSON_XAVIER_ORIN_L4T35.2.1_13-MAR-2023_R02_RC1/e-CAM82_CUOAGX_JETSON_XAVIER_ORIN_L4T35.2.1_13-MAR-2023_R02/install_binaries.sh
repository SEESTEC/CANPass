...
init ()
{
	echo "Enter Lane Congiguration mode to flash
	      1. 2lane
	      2. 4lane"
	read lane
	...
	set -eu
	MAX_PLATFORMS=6 # Current platforms supported: NANO, TX2, XAVIER, TX2-NX, XAVIER-NX, AGX-ORIN
	PLATFORMS=()
	echo "============================================="
...
update_kernel () {
	...
	if [ $JETSON_PLATFORM == "jetson-nano" ] || [ $JETSON_PLATFORM == "jetson-tx2" ] || [ $JETSON_PLATFORM == "jetson-tx2nx" ] || [ $JETSON_PLATFORM == "jetson-xavier" ] || [ $JETSON_PLATFORM == "jetson-xaviernx" ] || [ $JETSON_PLATFORM == "jetson-orin" ]; then
...
update_devicetree () {
	...
	if [ $JETSON_PLATFORM == "jetson-nano" ] ; then
		...
		DTB_DEVICE=$(readlink -f /dev/disk/by-partlabel/kernel-dtb);
                ...
                sudo cp $EXTRACTED_PATH/Kernel/tegra234-p3701-0000-p3737-0000-camera-${lane}* /boot/dtb/kernel_tegra234-p3701-0000-p3737-0000.dtb -f
                ...
                echo "Copying device-tree to /boot/dtb/ folder"
                sudo cp $EXTRACTED_PATH/Kernel/tegra194-p2888-0001-p2822-0000-camera-${lane}* /boot/dtb/kernel_tegra194-p2888-0001-p2822-0000.dtb -f
        elif [ $JETSON_PLATFORM == "jetson-xaviernx" ] ; then
                echo "Jetson-XavierNX : Taking backup of device-tree"
	...
	echo "Checking md5sum for dtb file";
        if [ $JETSON_PLATFORM == "jetson-orin" ] ; then
	        if [ $(md5sum $EXTRACTED_PATH/Kernel/tegra234-p3701-0000-p3737-0000-camera-${lane}* | cut -d " " -f1) == $(md5sum /boot/dtb/kernel_tegra234-* | cut -d " " -f1) ] ; then
        ...
        if [ $JETSON_PLATFORM == "jetson-xavier" ] ; then
        	if [ $(md5sum $EXTRACTED_PATH/Kernel/tegra194-p2888-0001-p2822-0000-camera-${lane}* | cut -d " " -f1) == $(md5sum /boot/dtb/kernel_tegra194-p2888-0001-p2822-0000.dtb | cut -d " " -f1) ] ; then
                	echo "Kernel dtb updated successfully";
	        else
        	        echo "DTB checksum Failed";
                	echo "Exiting";
...
update_modules () {
	...
	JP_VER=$(echo $JETPACK_VER | cut -c 3-5 );
	if (($(echo "$JP_VER > 4.6" | bc -q ))) ;then

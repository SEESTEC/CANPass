...
init ()
{
	...
	if [ $PRO_NAME -ne 20 ] && [ $PRO_NAME -ne 21 ] && [ $PRO_NAME -ne 25 ] && [ $PRO_NAME -ne 81 ]; then
		echo -e "\033[1;31mInvalid argument... This Package Support only 20, 21, 25 and 81"
		echo -e "\033[0;m"
		exit 1
	fi
...
prerequisites ()
{
	...
	echo "Checking Is ECAM_YUV_GMSL package is already installed..."
	REL_INT_FILE="/var/econ/release_integrity.md5"
	rel_pkg_name=$(md5sum -c release_integrity.md5);
	rel_pkg_name=$(echo $rel_pkg_name | cut -d":" -f1);
	if [ -f "$REL_INT_FILE" ]; then
		rootfs_pkg_name=$(md5sum -c /var/econ/release_integrity.md5);
		rootfs_pkg_name=$(echo $rootfs_pkg_name | cut -d":" -f1); 
		if [ "$rootfs_pkg_name" = "$rel_pkg_name" ]; then
			echo " Package is already Installed...!"
			echo " So Skipping Redundant Binaries Update...!"
			IS_UPDATE_BIN=0;
		else
			echo "Package is not installed previously!!!"
			cp release_integrity.md5 /var/econ/
			IS_UPDATE_BIN=1;
		fi
	else
		echo "Package is not installed previously!!!"
...
update_devicetree () {
	...
	if  [ $JETSON_PLATFORM == "jetson-orin" ] ; then
		...
		if [ $ORIN_PLATFORM == "0000" ]; then
			echo "Orin 32GB Dev Kit"
			sudo cp $EXTRACTED_PATH/Kernel/tegra234-p3737-0000+p3701-0000-nv.dtb /boot/dtb/kernel_tegra234-p3737-0000+p3701-0000-nv.dtb -f
		elif [ $ORIN_PLATFORM == "0005" ]; then
			echo "Orin 64GB Dev Kit"
			sudo cp $EXTRACTED_PATH/Kernel/tegra234-p3737-0000+p3701-0005-nv.dtb /boot/dtb/kernel_tegra234-p3737-0000+p3701-0005-nv.dtb -f
		fi
		echo "Copying DTB Overlay to /boot/ folder"
		if [ $PRO_NAME == "20" ] ; then
			echo "Copying NileCAM20/STURDeCAM20 DTB & Firmware ";
			sudo cp $EXTRACTED_PATH/Kernel/tegra234-p3737-camera-overlay_ar0230_two_lane.dtbo /boot/ -f
			sudo cp $EXTRACTED_PATH/firmware/ar0230_mcu_fw.bin /usr/lib/firmware -f

		elif [ $PRO_NAME == "21" ] ; then
			echo "Copying NileCAM21/STURDeCAM21 DTB & Firmware ";
			sudo cp $EXTRACTED_PATH/Kernel/tegra234-p3737-camera-overlay_ar0233_two_lane.dtbo /boot/ -f
			sudo cp $EXTRACTED_PATH/firmware/ar0233_mcu_fw.bin /usr/lib/firmware -f

		elif [ $PRO_NAME == "25" ] ; then
			echo "Copying NileCAM25/STURDeCAM25 DTB & Firmware ";
			sudo cp $EXTRACTED_PATH/Kernel/tegra234-p3737-camera-overlay_ar0234_two_lane.dtbo /boot/ -f
			sudo cp $EXTRACTED_PATH/firmware/ar0234_mcu_fw.bin /usr/lib/firmware -f

		elif [ $PRO_NAME == "81" ] ; then
			echo "Copying NileCAM81/STURDeCAM81 DTB & Firmware ";
			sudo cp $EXTRACTED_PATH/Kernel/tegra234-p3737-camera-overlay_ar0821_two_lane.dtbo /boot/ -f
			sudo cp $EXTRACTED_PATH/firmware/ar0821_mcu_fw.bin /usr/lib/firmware -f
		else
			echo "Error in DTB & Firmware copy Invalid Product!!!";
			exit 1;
		fi
	...
	if [ $JETSON_PLATFORM == "jetson-orin" ] ; then
		if [ $PRO_NAME == "20" ] ; then
			echo " checking md5sum of dtb ";
			if [ $(md5sum $EXTRACTED_PATH/Kernel/tegra234-p3737-camera-overlay_ar0230_two_lane.dtbo | cut -d " " -f1) == $(md5sum /boot/tegra234-p3737-camera-overlay_ar0230_two_lane.dtbo | cut -d " " -f1) ] ; then
				fdtdump /boot/tegra234-p3737-camera-overlay_ar0230_two_lane.dtbo > /tmp/as
			...
			echo " checking md5sum of dtb ";
			if [ $(md5sum $EXTRACTED_PATH/Kernel/tegra234-p3737-camera-overlay_ar0233_two_lane.dtbo | cut -d " " -f1) == $(md5sum /boot/tegra234-p3737-camera-overlay_ar0233_two_lane.dtbo | cut -d " " -f1) ] ; then
				sudo fdtdump /boot/tegra234-p3737-camera-overlay_ar0233_two_lane.dtbo > $HOME/as
			...
			echo " checking md5sum of dtb ";
			if [ $(md5sum $EXTRACTED_PATH/Kernel/tegra234-p3737-camera-overlay_ar0234_two_lane.dtbo | cut -d " " -f1) == $(md5sum /boot/tegra234-p3737-camera-overlay_ar0234_two_lane.dtbo | cut -d " " -f1) ] ; then
				fdtdump /boot/tegra234-p3737-camera-overlay_ar0234_two_lane.dtbo > /tmp/as
				sudo /opt/nvidia/jetson-io/config-by-hardware.py -n 2="$(echo $(cat /tmp/as |  grep -i "overlay-name") | cut -d '"' -f2 )";
				echo "Kernel dtb updated successfully";
			else
				echo "DTB checksum Failed";
			...
			echo " checking md5sum of dtb ";
			if [ $(md5sum $EXTRACTED_PATH/Kernel/tegra234-p3737-camera-overlay_ar0821_two_lane.dtbo | cut -d " " -f1) == $(md5sum /boot/tegra234-p3737-camera-overlay_ar0821_two_lane.dtbo | cut -d " " -f1) ] ; then
				fdtdump /boot/tegra234-p3737-camera-overlay_ar0821_two_lane.dtbo > /tmp/as
				sudo /opt/nvidia/jetson-io/config-by-hardware.py -n 2="$(echo $(cat /tmp/as |  grep -i "overlay-name") | cut -d '"' -f2 )";
				echo "Kernel dtb updated successfully";
			else
				echo "DTB checksum Failed";
			...
			echo "Error in DTB copy Invalid Product!!!";
			exit 1;
		fi
...
misc_installation () {
	if [ $IS_UPDATE_BIN == "0" ]; then
		echo "skipping misc installation...";
		return 0;
	fi
